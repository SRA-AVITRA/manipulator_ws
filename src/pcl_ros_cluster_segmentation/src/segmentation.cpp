...
public:
  ...
  explicit segmentation(ros::NodeHandle nh) : m_nh(nh)  {
    ...
    m_ros_pcl_Pub = m_nh.advertise<pcl::PointCloud<pcl::PointXYZ> ("obj_recognition/pcl_clusters",1);
...
void segmentation::cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
  ...
  pcl::toROSMsg(*CloudClusters,object_msg );

  m_ros_pcl_Pub.publish(object_msg);
