...
def image_callback(ros_rgb, ros_depth):
    ...
    print(HOME)
    cv2.imwrite(HOME+'/manipulator_ws/temp/color'+path+'.png',rgb)
    cv2.imwrite(HOME+'/manipulator_ws/temp/depth'+path+'.png',depth)
    color = cv2.imread(HOME+'/manipulator_ws/temp/color'+path+'.png')
    depth = cv2.imread(HOME+'/manipulator_ws/temp/depth'+path+'.png',0)
    color[:,:,0] = depth
    print("writing")
    cv2.imwrite(HOME+'/manipulator_ws/temp/rgd'+path+'.png',color)
