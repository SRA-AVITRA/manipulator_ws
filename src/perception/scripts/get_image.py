...
def image_callback(ros_rgb, ros_depth):
    ...
    cv2.imwrite(os.getenv("HOME")+'/manipulator_ws/temp/color'+path+'.png',rgb)
    cv2.imwrite(os.getenv("HOME")+'/manipulator_ws/temp/depth'+path+'.png',depth)
    color = cv2.imread(os.getenv("HOME")+'/manipulator_ws/temp/color'+path+'.png')
    depth = cv2.imread(os.getenv("HOME")+'/manipulator_ws/temp/depth'+path+'.png',0)
    color[:,:,0] = depth
    print("writing")
