...
def image_callback(ros_rgb, ros_depth):
    ...
    cv2.imwrite('color'+path+'.png',rgb)
    cv2.imwrite('depth'+path+'.png',depth)
    color = cv2.imread('color'+path+'.png')
    depth = cv2.imread('depth'+path+'.png',0)
    color[:,:,0] = depth
    print("writing")
    cv2.imwrite('rgd'+path+'.png',color)
